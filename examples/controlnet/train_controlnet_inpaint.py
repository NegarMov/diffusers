...
def import_model_class_from_model_name_or_path(pretrained_inpaint_model_name_or_path: str, revision: str):
    text_encoder_config = PretrainedConfig.from_pretrained(
        pretrained_inpaint_model_name_or_path,
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="train_controlnet_inpaint",
        help=(
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    if args.distorted_image_column is None:
        distorted_image_column = column_names[2]
        logger.info(f"distorted image column defaulting to {distorted_image_column}")
    else:
        distorted_image_column = args.distorted_image_column
        if distorted_image_column not in column_names:
            raise ValueError(
                f"`--distorted_image_column` value '{args.distorted_image_column}' not found in dataset columns. Dataset columns are: {', '.join(column_names)}"
            )
    ...
    distorted_image_transforms = transforms.Compose(
        [
            transforms.Resize(args.resolution, interpolation=transforms.InterpolationMode.BILINEAR),
            transforms.CenterCrop(args.resolution),
            transforms.ToTensor(),
    ...
    def preprocess_train(examples):
        ...
        distorted_images = [image.convert("RGB") for image in examples[distorted_image_column]]
        distorted_images = [distorted_image_transforms(image) for image in distorted_images]

        examples["pixel_values"] = images
        examples["conditioning_pixel_values"] = conditioning_images
        examples["distorted_pixel_values"] = distorted_images
...
def collate_fn(examples):
    ...
    distorted_pixel_values = torch.stack([example["distorted_pixel_values"] for example in examples])
    distorted_pixel_values = distorted_pixel_values.to(memory_format=torch.contiguous_format).float()
    ...
    return {
        ...
        "distorted_pixel_values": distorted_pixel_values,
...
def main(args):
    ...
    elif args.pretrained_inpaint_model_name_or_path:
    ...
    text_encoder_cls = import_model_class_from_model_name_or_path(args.pretrained_inpaint_model_name_or_path, args.revision)
    ...
    vae = AutoencoderKL.from_pretrained(args.pretrained_inpaint_model_name_or_path, subfolder="vae", revision=args.revision)
    inpaint_unet = UNet2DConditionModel.from_pretrained(
        args.pretrained_inpaint_model_name_or_path, subfolder="unet", revision=args.revision
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs): 
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                # prepare the image, mask, masked image, and the contition image
                flawless_image = batch["pixel_values"].to(dtype=weight_dtype)
                ...
                latents_image = latents_image * vae.config.scaling_factor
                _, _, height, width = latents_image.shape
                ...
                latents_masked_image = vae.encode(masked_image).latent_dist.sample()
                latents_masked_image = latents_masked_image * vae.config.scaling_factor

                # Sample noise that we'll add to the latents
                noise = torch.randn_like(latents_image)
                bsz = latents_image.shape[0]
                # Sample a random timestep for each image
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents_image.device)
                timesteps = timesteps.long()

                # Add noise to the latents according to the noise magnitude at each timestep
                # (this is the forward diffusion process)
                noisy_latents = noise_scheduler.add_noise(latents_image, noise, timesteps)

                latent_model_input = torch.cat([noisy_latents, latents_masked_image, mask], dim=1)
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    target = noise_scheduler.get_velocity(latents_image, noise, timesteps)
                else:
    ...
    if accelerator.is_main_process:
        ...
        if args.push_to_hub:
            save_model_card(
                ...
                base_model=args.pretrained_inpaint_model_name_or_path,
